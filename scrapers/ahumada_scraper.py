"""
Farmacias Ahumada scraper.
Scrapes medicine prices from www.farmaciasahumada.cl
"""
...
class AhumadaScraper(BaseScraper):
    """Scraper for Farmacias Ahumada."""
    
    def __init__(self):
        """Initialize Ahumada scraper."""
        settings = get_settings()
        super().__init__(
            pharmacy_name="Farmacias Ahumada",
            base_url=settings.farmacias_ahumada_url
        )
    
    def _build_search_url(self, query: str) -> str:
        """
        Build search URL for Farmacias Ahumada.
        ...
        return f"{self.base_url}/buscar?q={encoded_query}"
    
    def _find_product_elements(self, soup: BeautifulSoup) -> List[Any]:
        """
        Find product elements in Ahumada HTML.
        ...
        # Try common selectors for Ahumada
        selectors = [
            "div.product-item",
            "div.product-card",
            "article.product",
    ...
    def _parse_product(self, element: Any) -> Optional[ProductResult]:
        """
        Parse a product element from Ahumada.
        ...
        try:
            ...
            logger.warning(f"Failed to parse Ahumada product: {e}")
            return None
    ...
    def _extract_sku_from_element(self, element: Any) -> Optional[str]:
        """Extract SKU."""
        for attr in ["data-sku", "data-product-id", "data-id", "data-pid"]:
            if element.get(attr):
    ...
    async def search(self, query: str, max_results: int = 20) -> ScraperResult:
        """
        Search for products in Farmacias Ahumada.
